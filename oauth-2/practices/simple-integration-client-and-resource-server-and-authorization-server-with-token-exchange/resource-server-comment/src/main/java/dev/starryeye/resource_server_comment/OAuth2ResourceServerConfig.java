...
public class OAuth2ResourceServerConfig {
     ...
     *      aud 에 이 서버의 식별자(자원 URI.. http://localhost:8082)가 있어야만 수락하도록 validator 를 더한다.
     *          (authorization server 의 customizer 가 token exchange 시 audience 파라미터를 aud 로 반영해 이 값이 들어온다)
    ...
    public JwtDecoder jwtDecoder(@Value("${spring.security.oauth2.resourceserver.jwt.jwk-set-uri}") String jwkSetUri) {
        ...
        OAuth2TokenValidator<Jwt> audienceValidator = new JwtClaimValidator<List<String>>(
                JwtClaimNames.AUD, audience -> audience != null && audience.contains("http://localhost:8082"));
