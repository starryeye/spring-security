...
`token-state` 소비자는 `ack-mode: record` 다 — 이것은 애플리케이션이 직접 호출하는 "수동 커밋"이 아니라
Spring Kafka 컨테이너가 레코드 하나를 처리할 때마다 자동으로 커밋하는 모드다. 그리고 그 커밋은
"처리 성공 후"로 한정되지 않는다 — 재시도가 소진돼 `DeadLetterPublishingRecoverer` 가 레코드를 DLT 로
넘기면(recovery), 컨테이너는 그 레코드도 "처리됐다"로 보고 오프셋을 커밋한다. 그래서 더 정확히는
"레코드 하나에 대한 리스너 사이클(성공 또는 DLT 로의 recovery)이 끝날 때마다 커밋"이다. **무한 재시도로 일부러 설정하면** 그 파티션의 뒷 이벤트가 전부 막힌다(head-of-line blocking) — 한 사용자의 폐기가 안 되는 동안 다른 사용자들 폐기까지 멈춘다.
...
| **Kafka** | 편지가 outbox 에 쌓인다. 살아나면 나가서 폐기된다. **지연되지만 결국 된다**(단, 아래 조건 참고) |
...
**"Kafka 가 살아나면 나가서 폐기된다"는 outbox 의 행 하나하나가 유한 시간 안에 발행을 재시도한다는
전제 위에서만 참이다.** `OutboxPublisher.publishPending()` 은 한 행의 발행이 실패하면(순서 보존을 위해)
그 자리에서 즉시 멈추고 다음 주기를 기다린다 — 실패 사유가 "Kafka 가 잠깐 죽어 있었다"처럼 시간이
지나면 풀리는 것이면 그 행은 결국 나간다. 하지만 그 실패가 **영구적**이면(예: `row.getPayload()` 가
어떤 이유로 브로커가 항상 거부하는 형태다) 그 행은 매 주기 똑같이 실패하고, 그 뒤에 쌓인 모든 미발행
행(다른 세션의 로그아웃 포함)도 함께 영원히 멈춘다 — 로그(`log.warn`)만 남을 뿐 별도 알림·우회 경로가
없다. 이 슬라이스는 그런 영구 실패 행이 생기지 않는다고 가정할 뿐 그 가정을 검증하거나 막지 않는다
(`OutboxPublisher` javadoc 참고).
...
**재현된 실패 (직접 발행 단계) — 역사적 기준. T6 이 세웠고 T7 재검토로 6번은 뒤집혔다. 아래 §6 "재현
단계 — 직접 발행 먼저" 의 두 "주의" 문단이 정정판이다. 이 기준을 직접 뒷받침하던 `DirectPublishFailureModeTest`
는 outbox 버전(`OutboxFailureModeTest`)으로 교체되며 T7 에서 삭제됐다 — 지금 트리에는 이 두 기준을
직접 재현하는 테스트가 없다.**

6. ~~Kafka 정지 상태로 로그아웃 → 세션은 지워지고 refresh 는 살아남는다~~ → **정정(T7): `publish()` 가
   `@Transactional` 메서드 안에서 블로킹으로 불려 예외가 트랜잭션 프록시까지 올라가므로, 트랜잭션
   전체가 롤백된다 — 세션 행도 지워지지 않고 refresh 도 죽지 않는다. 최종 결과(둘 다 살아남는다)는
   원래 예측과 같지만, "세션은 지워지고 refresh 만 산다"는 경로 서술은 틀렸다
7. 커밋 실패 주입 → 세션은 남는데 refresh 만 죽는다 — **T6 은 이 기준을 실제로 재현하지 않았다.**
   `publishHappensBeforeCommit` 은 "발행이 커밋 전에 일어난다"는 전제만 확인했을 뿐 커밋을 실제로
   실패시키지 않았다. 그 전제로부터 이 기준이 원리적으로 가능하다는 추론만 남아 있을 뿐, 관찰로
   고정된 적은 없다
...
- **회전-폐기 경쟁** — `RefreshTokenService.revokeBySid` 는 계열(family) 잠금을 쓰지 않는다. 이 연산이
  보장하는 것은 "UPDATE 문이 행을 평가하는 순간 sid 가 일치하고 ACTIVE 인 행을 폐기한다"뿐이라, 회전이
  동시에 새 ACTIVE 형제 행을 삽입·커밋하는 시점이 이 UPDATE 의 스캔 전이냐 후냐에 따라 그 행이 폐기
  대상에서 빠질 수 있다. 놓친 행은 로그아웃 뒤에도 살아남고, 이 호출 하나로는 재검증·재시도가 일어나지
  않는다 — 별도의 정합성 스윕이 있어야 닫히는데, 이 슬라이스는 그것을 제공하지 않는다
  (`RefreshTokenService.revokeBySid` javadoc 참고)
