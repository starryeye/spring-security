...
`token-state` 소비자는 수동 커밋(처리 성공 후 ack)이다. 그런데 무한 재시도를 걸면 **그 파티션의 뒷 이벤트가 전부 막힌다**(head-of-line blocking). 한 사용자의 폐기가 안 되는 동안 다른 사용자들 폐기까지 멈춘다.

제한 재시도 후 `oidc.session.logged-out.v1.dlt` 로 보내고 로그를 남긴다. 실패가 조용히 사라지지 않으면서 스트림은 계속 흐른다.
...
|---|---|---|
| **유실** — Kafka 정지 후 로그아웃 | 세션은 지워지는데 refresh 가 살아남는다 | 편지가 outbox 에 남아, Kafka 를 올리면 그때 나가서 폐기된다 |
| **유령** — 커밋 실패 주입 | 세션은 남는데 refresh 만 죽는다 | 편지도 함께 롤백돼 아예 안 나간다 |

두 실패를 **먼저 통과하는 테스트로 고정**하고, outbox 로 바꾼 뒤 그 테스트가 반대 결과를 요구하도록 뒤집는다. "outbox 가 필요하다"가 설명이 아니라 **재현된 실패**로 남는다.
...
| 특정 편지만 계속 실패 | 제한 재시도 후 DLT. 그 한 건은 처리 안 되고 로그가 남는다. **뒷줄은 안 막힌다** |
