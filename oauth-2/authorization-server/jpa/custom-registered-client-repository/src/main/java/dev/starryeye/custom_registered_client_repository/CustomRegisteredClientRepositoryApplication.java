...
public class CustomRegisteredClientRepositoryApplication {
	 ...
	 *      기존 프로젝트들은 "{noop}secret" 로 저장했지만.. 여기서는 "{bcrypt}..." 로 인코딩하여 DB 에 저장해본다.
	 *          client 는 여전히 raw secret("secret") 으로 인증 요청하면 된다.
	 *
	 * 확인 포인트
	 *      1. authorization code grant : bcrypt 로 저장된 secret 으로 client_secret_basic 인증이 성공한다.
	 *      2. PKCE (my-public-client) : ClientSettings 의 requireProofKey 가 JSON 왕복 후에도 동작한다.
	 *      3. token 응답의 expires_in : TokenSettings 의 accessTokenTimeToLive(120초) 가 JSON 왕복 후에도 반영된다. (기본값 300초)
	 *      4. 서버 재기동 후에도 client 3개가 유지된다.
	 *
	 * 실행 방법
	 *      1. docker-compose/docker-compose.yml 로 MySQL 기동
	 *      2. 서버 기동 후 http/ 의 .http 파일로 grant 별 flow 수행
	 *      3. "/registered-clients", "/registered-clients/raw" 로 저장 상태 관찰 (http/api.http)
	 */
