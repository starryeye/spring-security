...
public class AuthorizationServerConfig {
    ...
    /**
     * 이 프로젝트의 주제는 세션 외부화(spring-session-data-redis)이다.
     *      세션 관련 설정 코드는 이 프로젝트에 한 줄도 없다.. 의존성과 redis 접속 설정만으로 자동 구성되는 것이 포인트. (main class 주석 참고)
     *      나머지는 기존 프로젝트들과 동일한 boilerplate (저장소 전부 InMemory/기본값).
     *
     * 참고. issuer 를 8091 로 고정해두고 두번째 인스턴스(8092)도 같은 설정을 쓴다.
     *      운영에서 다중 인스턴스는 모두 같은 issuer(LB 주소)를 사용하므로 이것이 실제 모습에 가깝다.
     */
