...
```
client → gateway → token: GET /userinfo (Authorization: Bearer {access_token})
① access token 검증 — 서명(signing jwks), exp, iss
② openid scope 보유 확인 (없으면 403 insufficient_scope)
③ 토큰의 sub 로 user-directory 조회
④ scope 기반 필터링 후 application/json 반환
```
...
| `auth_time` | 로그인 시각(epoch seconds) | max_age·essential 시 필수 — 이 구현은 항상 포함(허용됨) |
...
| userinfo 무효 토큰(서명/만료/iss) | 401 `invalid_token` | token |
...
- **user-directory 다운** → id token 은 **프로필 claim 없이 발급**하고(필수 claim 만으로 표준상 유효), userinfo 는 503 으로 실패시킨다. 인증 자체(누가 로그인했는가)는 성공시키고, 프로필 조회라는 부가 기능만 degrade 한다.
- **signing 다운** → 슬라이스 1 과 동일. access token·id token 발급 불가, 이미 발급된 JWT 의 검증은 캐시된 jwks 로 계속된다.
...
- AuthorizeController MockMvc 통합 테스트 보강
