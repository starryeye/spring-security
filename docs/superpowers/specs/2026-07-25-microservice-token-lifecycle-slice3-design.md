...
- 같은 토큰으로 **동시 회전 두 건**(`RefreshTokenServiceConcurrentRotateTest`) → 정확히 하나만 `ROTATED`, 다른 하나는 `REUSE_DETECTED` 이고 제출된 토큰이 DB 상 REVOKED(`revoked_reason=REUSE_DETECTED`)임을 확인한다. `TokenGenerator.hash` 에 장벽을 세워 두 트랜잭션이 각자의 첫 조회를 마친 뒤에야 잠금 경쟁에 들어가도록 인터리빙을 강제한다.
  **한계(h2).** "계열 전 행이 REVOKED" 는 이 테스트(h2)에서 단언하지 않는다. h2 에서 관찰되는 동작은 이렇다 — 늦게 잠근 쪽의 차단됐던 `SELECT ... FOR UPDATE` 가 풀릴 때, **기다리던 그 행 자체는 다시 읽어 최신 커밋 상태(CONSUMED)를 반영한다**(그래서 REUSE_DETECTED 로 판정할 수 있다). 하지만 **대기하는 동안 다른 트랜잭션이 새로 삽입·커밋한 형제 행까지 이번 조회의 행 집합에 포함하지는 않는다** — 그래서 이 테스트에서는 그 형제 행 하나가 ACTIVE 로 남을 수 있다. 이것은 구현의 결함이 아니라 테스트 DB(h2)에서 관찰된 잠금 의미론이다. "**잠글 때 이미 존재하던** 행 전부가 폐기된다"는 이미 커밋된 행으로만 구성해 h2 에서도 성립하는 위 단일 스레드 테스트(`reusingConsumedTokenAfterMultipleRotationsRevokesEveryFamilyMember`)가 고정한다. 이 동시성 테스트(h2)가 실제로 고정하는 것은 "회전이 정확히 한 번만 일어났다"(계열 행 개수가 3이 아니라 2)와 "제출된 토큰이 재사용으로 폐기됐다" 두 가지뿐이다.
  **MySQL(InnoDB) 로 검증됨.** "대기 중 다른 트랜잭션이 삽입한 형제 행도 폐기되는가"는 `RefreshTokenServiceMySqlLockSemanticsTest`(별도 프로필 `application-mysql-verify.yml`, 격리 스키마 `token_state_test`)가 실제 MySQL 8/InnoDB 로 검증한다. 같은 barrier 기법으로 인터리빙을 강제한 뒤, 승자(ROTATED)가 삽입·커밋한 형제 행의 최종 상태까지 확인한다. **반복 실행(연속 16회) 전부 그 형제 행이 REVOKED 로 함께 폐기됨을 확인했다** — h2 의 관찰과 달리 InnoDB 에서는 잠금 대기가 풀린 뒤의 `findByFamilyIdForUpdate` 재조회가 대기 중 삽입된 형제 행을 항상 포함했다. h2 의 결과는 그래서 구현 결함이 아니라 h2 고유의 잠금 재조회 범위 한계로 결론 내린다. 이 테스트는 MySQL 이 없는 환경(로컬·CI)에서는 `@BeforeAll` 의 `Assumptions.assumeTrue` 로 스킵되고 사유가 표준 출력에 남는다 — 전체 빌드를 깨지 않는다.
- `family_expires_at` 초과 → `EXPIRED` (개별 `expires_at` 은 아직 유효한 상태로 구성해 절대 상한이 실제로 동작하는지 격리 검증)
- 개별 `expires_at` 초과(계열 상한은 아직 미래) → `EXPIRED`, introspect 는 `active: false` (`rotateWithExpiredIndividualTokenButFamilyStillValidReturnsExpired`, `introspectReturnsInactiveWhenIndividualTokenExpiredButFamilyStillValid`) — 위 항목의 반대 격리다. `isExpired` 의 개별 만료 항을 잠시 무력화해 두 테스트가 실제로 실패하는지 확인한 뒤 원복해 고정했다.
